...
import { calculateSRP_A, calculatePasswordVerifier } from '@/utils/auth';
...
export class CognitoAuthService {
  ...
  async signIn(username: string, password: string): Promise<AuthTokens> {
    try {
      // Step 1: SRP_A を計算
      const { SRP_A, authenticationHelper } = await calculateSRP_A(this.config.userPoolName);
      ...
      if (challengeResponse.ChallengeName === ChallengeNameType.NEW_PASSWORD_REQUIRED) {
  ...
  private async initiateAuth(username: string, srpA: string): Promise<InitiateAuthCommandOutput> {
  ...
  private extractChallengeParameters(response: InitiateAuthCommandOutput): ChallengeParameters {
    ...
    const missingParams = requiredParams.filter(param => !challengeParams[param]);
  ...
  private extractTokens(response: RespondToAuthChallengeCommandOutput): AuthTokens {
