...
### Before
- **Frontend**: Uses manually entered user names (e.g., "****", "****") as user IDs
...
1. **Frontend-only users**: Users who only created entries via the dashboard
   - User IDs are display names (e.g., "****", "****")
...
For existing display name users (e.g., "****", "****"):
...
3. If not, system presents a one-time setup screen:
   - "We found existing data under the name: [****, ****, etc.]"
...
5. System migrates data:
   ...
   - Updates all COST_DATA items: Changes PK from `USER#****` to `USER#{cognitoId}`
...
1. Have existing data under display name "****"
