...
export const monthlyDashboardHandler = async (c: Context) => {
  try {
    ...
    if (!year || !month) {
      const errorResponse = errorResponseSchema.parse({ error: "yearMonth parameter is required" });
...
export const userDetailsHandler = async (c: Context) => {
  try {
    ...
    if (!/^\d{4}-\d{2}$/.test(yearMonth)) {
      const errorResponse = errorResponseSchema.parse({ error: "yearMonth must be in YYYY-MM format" });
      return c.json(errorResponse, 400);
    }
...
export const categorySummaryHandler = async (c: Context) => {
  try {
    ...
    const validatedCategorySummary = categorySummaryResponseSchema.parse(categorySummary);
