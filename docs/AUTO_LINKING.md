...
1. **User interacts with LINE Bot**
   - Selects display name "****" during bot interaction
...
2. **Same user logs into dashboard with Cognito**
   ...
   - Provides display name "****" (or leaves it empty, defaulting to email)
   - System searches for unlinked LINE users with display name "****"
...
Display Name: **** (provided or from email)
Action: Create cost ¥50,000

System checks for unlinked LINE users with "****"
Finds: U111222333
